...
class GenerationMixin:
    ...
    def _prepare_decoder_input_ids_for_generation(
        self, input_ids: torch.LongTensor, decoder_start_token_id: int = None, bos_token_id: int = None, decoder_prompt=None
    ) -> torch.LongTensor:
        decoder_input_ids = [self._get_decoder_start_token_id(decoder_start_token_id, bos_token_id)]
        if decoder_prompt is not None:
            decoder_input_ids+=decoder_prompt
        decoder_input_ids=torch.tensor([decoder_input_ids],dtype=torch.long, device=input_ids.device)
        decoder_input_ids=decoder_input_ids.expand(input_ids.shape[0],decoder_input_ids.shape[-1])
    ...
    def _expand_inputs_for_generation(
        ...
        **model_kwargs,
        ...
        if is_encoder_decoder:
            ...
            #$# 加入对cross attention的expand
            encoder_outputs['cross_attention_mask'] = encoder_outputs['cross_attention_mask'].index_select(0, expanded_return_idx)
    ...
    def generate(
        ...
        decoder_prompt:list=None,#$# 解码时加入prompt，为id列表形式
        **model_kwargs,
        ...
        if self.config.is_encoder_decoder:
            ...
            if "decoder_input_ids" in model_kwargs:
                ...
                input_ids = self._prepare_decoder_input_ids_for_generation(
                    input_ids, decoder_start_token_id=decoder_start_token_id, bos_token_id=bos_token_id,decoder_prompt=decoder_prompt
                )
        ...
        **model_kwargs,
        ...
        while True:
            ...
            if synced_gpus:
                ...
                if this_peer_finished_flag.item() == 0.0:
                    break
            #$#model_kwargs['encoder_outputs']里的结果是由各模型自身决定的，所以可以在这里面存入cross attention mask
            #修改模型对encoder_outputs的利用即可
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
